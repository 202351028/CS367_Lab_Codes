...
def policy_evaluation(policy,lambda_rent, lambda_ret,params, theta=1e-3, max_iters=1000):
    ...
    while True:
        ...
        for s1 in range(max_bikes+1):
            for s2 in range(max_bikes+1):
                a = int(policy[s1, s2])
                # clip action feasibly
                a = int(max(-max_move, min(max_move, a)))
                a = int(max(-s2, min(s1, a)))  # ensure result feasible
                # bikes after move
                s1_after_move = clamp(s1 - a, 0, max_bikes)
                s2_after_move = clamp(s2 + a, 0, max_bikes)

                # immediate deterministic costs
                if a > 0:
                ...
                for (ns1, ns2), prob in next_state_probs:
                    exp_future_v += prob * V_old[ns1, ns2]

                new_v = immediate_expected_reward + gamma * exp_future_v
                delta = max(delta, abs(new_v - V[s1, s2]))
                V[s1, s2] = new_v
